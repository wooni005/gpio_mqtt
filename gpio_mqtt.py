...
def on_connect(_client, userdata, flags, rc):
    if rc == 0:
        ...
        _client.subscribe([(settings.MQTT_TOPIC_OUT, 1), (settings.MQTT_TOPIC_COMMAND, 1), (settings.MQTT_TOPIC_CHECK, 1)])
    else:
...
def on_message_command(_client, userdata, msg):
...
def getOutputIndex(_topic, status):
...
def setOutputPin(_pinNr, val):
    if val != 0:
        sendQueueBoard2.put("1,%do".encode() % _pinNr)
    else:
        sendQueueBoard2.put("0,%do".encode() % _pinNr)
...
def on_message_output(_client, userdata, msg):
    global pinsEnabled
    global gpioTopics
    global pinStatus
    ...
    try:
        if pinsEnabled:
            ...
            if deviceName[0:10] == 'Boiler-SSR':  # Topic name
                ...
                outputIndex = getOutputIndex(_topic, status)
                if not outputIndex:
                    print(("ERROR: OutputPin Topic not found: " + _topic))
                    return
                else:
                    pinStatus[_topic] = status
                    if int(status) != 0:
                        # print("Pin High")
                    ...
                    # print(("on_message_output: " + _topic + " " + str(status)))

    except Exception as _e:
        print("on_message_output: er gaat iets fout, reden: %s" % str(_e))
        traceback.print_exc()
        return
...
def processGPIOinput(boardId, pinIndex, pinName, value):
    _topic = getTopic(boardId, pinIndex)

    # Check if the pin is existing in gpioTopics
    if _topic:
        pinStatus[_topic] = value
        ...
        if pinsEnabled:
            if value == 0:
                ...
                mqtt_publish.single(_topic, value, qos=1, hostname=settings.MQTT_ServerIP)
            else:
                mqtt_publish.single(_topic, value, qos=1, hostname=settings.MQTT_ServerIP)
...
def processBMP085(pressureAtStation, temp):
    ...
    sensorData = {'Sealevel': round(pressureSealevel / 100.0, 2), 'AtStation': round(pressureAtStation / 100.0, 2)}
